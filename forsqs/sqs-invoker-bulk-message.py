...
"""
Credit to https://github.com/timothymugayi/boto3-sqs-extended-client-lib  
Copied as part of Udemy course: 
...
class SQSClientExtended(object):
    """
    Stores config state, allows creation of service clients, resources
    :param aws_access_key_id -> string: AWS access key ID
    :param aws_secret_access_key -> string: AWS secret access key
    :param aws_session_token -> string: AWS temporary session token
    :param region_name -> botocore.session.Session: Default region
    :param botocore_session -> string: Use this session vs creating new one.
    :param profile_name -> string: The name of a profile to use.
    """

    def __init__(self, aws_access_key_id=None, aws_secret_access_key=None,
                 aws_region_name=None, s3_bucket_name=None):
    ...
    def __get_msg_attributes_size(self, message_attributes):
        ...
        for key, entry in message_attributes.items():
            total_msg_attributes_size += self.__get_string_size_in_bytes(key) # just the key
...
# TODO: Test this; it looks like it would not work
    ...
    def __is_s3_receipt_handle(self, receipt_handle):
        return True if SQSExtendedClientConstants.S3_BUCKET_NAME_MARKER.value in receipt_handle and\
                       SQSExtendedClientConstants.S3_KEY_MARKER.value in receipt_handle else False
    ...
    def send_message(self, queue_url, message, message_attributes={}):
        ...
        if msg_attributes_size > self.message_size_threshold:
            raise ValueError("Total size of Message attributes is {} bytes which is larger than the "
                             "threshold of {} Bytes. Consider including the payload in the message body instead of"
                             " message attributes.".format(msg_attributes_size, self.message_size_threshold))
        ...
        if message_attributes_number > SQSExtendedClientConstants.MAX_ALLOWED_ATTRIBUTES.value:
            raise ValueError("Number of message attributes [{}}] exceeds the maximum allowed"
                             " for large-payload messages [{}].".format(message_attributes_number,
        ...
        if large_payload_attribute_value:
            raise ValueError("Message attribute name {} is reserved for"
                             " use by SQS extended client.".format(SQSExtendedClientConstants.RESERVED_ATTRIBUTE_NAME.value))
